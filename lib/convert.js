...
export async function convertDocument(filePath, data, sniff, options) {
    ...
    if (options.markitdownBin !== undefined && options.markitdownBin !== '') {
        try {
            ...
            console.warn(`[dsh-chat-enhancements] MarkItDown CLI failed for ${filePath}, falling back to bundled engine:`, err);
        }
    }
    try {
        ...
        console.warn(`[dsh-chat-enhancements] bundled engine failed for ${filePath}, falling back to JS parsers:`, err);
    }
