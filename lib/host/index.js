...
import { describeImage } from "./attachments/vision.js";
import { defineReadDocumentTool, ParseCache } from "./attachments/tool.js";
import { createUploadHandler, createSweeper } from "./attachments/upload.js";
import { probeMarkitdown } from "./attachments/convert.js";
...
export const Config = z.object({
    ...
    /** When true, uploading a document also appends an `@filename` reference at the end of the composer draft (Gemini-style auto-attach). */
    attachReferences: z.boolean().default(false),
...
function assertPositiveInteger(value, label) {
    if (!Number.isInteger(value) || value < 1)
        throw new Error(`dsh-chat-enhancements: ${label} must be a positive integer`);
}
/**
...
export function apply(ctx, config) {
    ...
    const markitdown = () => {
        markitdownReady ??= resolveMarkitdownBin(config.markitdownBin).then((bin) => {
            ...
            if (bin !== '') {
                ...
                console.log('[dsh-chat-enhancements] Document → Markdown ready: bundled MarkItDown engine (20+ formats, image OCR) — fully packaged, no downloads, no Python.');
            }
            return bin;
        });
    ...
    const vision = async (filePath, name) => {
